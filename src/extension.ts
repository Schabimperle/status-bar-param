...
function addWorkspaceFolder(workspaceFolder: WorkspaceFolder) {
	...
	workspaceInputFiles.forEach((workspaceInputFile) => {
		const uri = Uri.joinPath(workspaceFolder.uri, workspaceInputFile);
		jsonFiles.push(new JsonFile(priority--, uri, workspaceFolder));
	});
}

function addJsonFile(path: Uri) {
	console.debug('addJsonFile', path.toString());
	jsonFiles.push(new JsonFile(priority--, path));
}
